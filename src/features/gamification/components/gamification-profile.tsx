...
const TEAM_QUALITY_ICONS = [
  { id: "leadership", icon: Shield, label: "Leadership", description: "Natural leader and mentor" },
  { id: "creativity", icon: Palette, label: "Creativity", description: "Innovative problem solver" },
  { id: "technical", icon: Code, label: "Technical", description: "Strong technical skills" },
  { id: "collaboration", icon: Users, label: "Collaboration", description: "Excellent team player" },
  { id: "focus", icon: Target, label: "Focus", description: "Goal-oriented achiever" },
  { id: "adaptability", icon: Settings, label: "Adaptability", description: "Flexible and versatile" },
];

const GAMIFICATION_ROLES = ["Developer", "Designer", "Project Manager", "Team Lead"];
...
export const GamificationProfile = ({ user, gamificationData, onUpdate, onPurchaseBadge }: GamificationProfileProps) => {
  ...
  const getSelectedIconComponents = () => {
    return selectedIcons.map(iconId => {
      ...
      return (
        <div key={iconId} className="flex flex-col items-center justify-between h-16 relative group">
          <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center relative cursor-pointer"
          ...
          <span className="text-xs text-gray-600 text-center">{iconData.label}</span>
        </div>
      );
    });
  };
                  ...
                  {selectedIcons.length < 3 && (
                    <Dialog open={isIconDialogOpen} onOpenChange={setIsIconDialogOpen}>
                      ...
                      <DialogContent className="max-w-2xl">
                        <DialogHeader>
                          <DialogTitle>Choose Your Team Qualities (Select up to 3)</DialogTitle>
                        </DialogHeader>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 py-4">
                          {TEAM_QUALITY_ICONS.map((iconData) => {
                            const IconComponent = iconData.icon;
                            const isSelected = selectedIcons.includes(iconData.id);
                            return (
                              <div
                                key={iconData.id}
                              ...
                              >
                                <div className="flex flex-col items-center space-y-2">
                                  ...
                                  <h3 className="font-medium text-sm">{iconData.label}</h3>
                                  <p className="text-xs text-gray-600 text-center">{iconData.description}</p>
                                </div>
                              </div>
                            );
                          })}
                        ...
                        <div className="flex justify-end space-x-2">
                          <Button variant="outline" onClick={() => setIsIconDialogOpen(false)}>
                            Cancel
                          </Button>
          ...
          {isEditingAbout ? (
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-gray-700">
                {gamificationData.aboutMe || "No description added yet. Click the edit button to add information about yourself."}
              </p>
            </div>
          )}
            </div>
